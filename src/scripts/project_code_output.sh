...
# Add directory tree information for /src
echo -e "\n=== Directory Structure: /src ===" >> "$OUTPUT"
cd /workspaces/mware3/src && tree >> "$OUTPUT"
echo "----------------------------------------" >> "$OUTPUT"

# Add directory tree information for /views
echo -e "\n=== Directory Structure: /views ===" >> "$OUTPUT"
if [ -d "/workspaces/mware3/views" ]; then
    cd /workspaces/mware3/views && tree >> "$OUTPUT"
else
    echo "Views directory not found" >> "$OUTPUT"
fi
echo "----------------------------------------" >> "$OUTPUT"
...
# Add all files from /views directory (if it exists)
if [ -d "/workspaces/mware3/views" ]; then
    echo -e "\n=== Adding files from /views directory ===" >> "$OUTPUT"
    find /workspaces/mware3/views -type f -exec sh -c '
        echo -e "\n=== File: {}" >> '$OUTPUT'
        cat {} >> '$OUTPUT'
        echo "----------------------------------------" >> '$OUTPUT'
    ' \;
